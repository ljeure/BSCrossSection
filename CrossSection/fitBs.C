...
void fitBs(int usePbPb=0,
    TString inputdata="/data/wangj/Data2015/Bntuple/pp/ntB_EvtBase_20160420_BfinderData_pp_20160419_bPt0jpsiPt0tkPt0p5.root",
    TString inputmc="/data/HeavyFlavourRun2/MC2015/Bntuple/pp/Bntuple20160606_pp_Pythia8_BuToJpsiK_Bpt5p0_Pthat5.root",
    ...
    TString cutmcgen="TMath::Abs(Gy)<2.4&&abs(GpdgId)==521&&GisSignal==1",
    int isMC=0, Double_t luminosity=1., int doweight=0, TString collsyst="PbPb",
    TString outputfile="", TString npfit="0", int doDataCor = 0,
  ...
  double ErrorOnSigma(double width, double errwidth, double smear,
      double errsmearing);
  ...
  if(!isPbPb) {
    ...
    seldata = Form("%s&&%s&&hiBin>=%f&&hiBin<=%f",trgselection.Data(),
        cut.Data(),hiBinMin,hiBinMax);
    selmceff = Form("%s&&hiBin>=%f&&hiBin<=%f",cut.Data(),hiBinMin,hiBinMax);
    selmcgen = Form("%s&&hiBin>=%f&&hiBin<=%f",cutmcgen.Data(),
        hiBinMin,hiBinMax);
    }
  ...
  TF1* fit (TTree* nt, TTree* ntMC, double ptmin, double ptmax, int isMC,bool,
      TF1* &total,Float_t centmin, Float_t centmax, TString npfit);
  ...
  if(doDataCor == 1){
    if(collisionsystem=="pp"||collisionsystem=="PP"
        ||collisionsystem=="ppInc"||collisionsystem=="PbPbInc")
      weightdata="1";
    else
      weightdata="1";
  }
  ...
  nt->AddFriend("ntSkim");
  if(!isPbPb) {
    nt->AddFriend("BDT_Tree");
    nt->AddFriend("MLP_Tree");
  }
  ...
  if(!isPbPb) {
    ntMC->AddFriend("BDT_Tree");
    ntMC->AddFriend("MLP_Tree");
  }
  ...
  for(int i=0;i<_nBins;i++) {
    TF1* f = fit(nt,ntMC,_ptBins[i],_ptBins[i+1],isMC,isPbPb,
        totalmass,centmin, centmax, npfit);
    ...
    double yieldErr = f->Integral(minhisto,maxhisto)/
      binwidthmass*f->GetParError(0)/f->GetParameter(0);
  ...
  ntMC->Project(
      "hPtMC","Bpt",TCut(weight)*(TCut(selmceff.Data())&&"(Bgen==23333)"));
  ...
  hPtSigma->SetTitle(";B_{s} p_{T} (GeV/c);d#sigma(B^{+})/dp_{T} (pb/GeV)");
...
TF1 *fit(TTree *nt, TTree *ntMC, Double_t ptmin, Double_t ptmax, int isMC,
  ...
  // fitting function that wasn't working (probably just a normalization error)
//  TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*TMath::Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+ [3]+[4]*x+[5]*x*x + [9]*("+iNP+") + [10]*0 ",
  TF1 *f = new TF1(Form("f%d",count),"[0]*([6]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2]) + [3]+[4]*x+[5]*x*x)",
      minhisto, maxhisto);

  if(isMC==1) {
    nt->Project(Form("h-%d",count),"Bmass", 
        Form("%s*(%s&&Bpt>%f&&Bpt<%f)*(1/%s)","1",seldata.Data(),
          ptmin,ptmax,weightdata.Data()));   
  }
  else { 
    nt->Project(Form("h-%d",count),"Bmass",Form("(%s&&Bpt>%f&&Bpt<%f)*(1/%s)",
          seldata.Data(),ptmin,ptmax,weightdata.Data()));   
  }
  ...
  h->Fit(Form("f%d",count),"q 0","",minhisto,maxhisto);
  h->Fit(Form("f%d",count),"q 0","",minhisto,maxhisto);
  h->Fit(Form("f%d",count),"q 0","",minhisto,maxhisto);

  // mass fitting function that wasn't working 
  // // (probably just a normalization error)
//  TF1 *mass = new TF1(Form("fmass",count),"[0]*([3]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[3])*TMath::Gaus(x,[1],[4])/(sqrt(2*3.14159)*[4]))",
 //     minhisto, maxhisto);
 //
  TF1 *mass = new TF1(Form("f%d",count),"[0]*([3]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2]))",
      minhisto, maxhisto);

  mass->SetParameters(f->GetParameter(0),
  ...
  // as of 9/1/2017 this commented out code wsa causing problems with fitting.
  // It might just be a normalization problem
  /*
  ...
  std::cout << "f param 0 " << f->GetParameter(0) << std::endl;
  std::cout << "f param 1 " << f->GetParameter(1) << std::endl;
  std::cout << "f param 2 " << f->GetParameter(2) << std::endl;
  std::cout << "f param 7 " << f->GetParameter(7) << std::endl;
  std::cout << "f param 8 " << f->GetParameter(8) << std::endl;

  mass->SetParameters(f->GetParameter(0),
      f->GetParameter(1),f->GetParameter(2),
      f->GetParameter(7),f->GetParameter(8));
  
  mass->FixParameter(0, f->GetParameter(0));
  mass->FixParameter(1, f->GetParameter(1));
  mass->FixParameter(2, f->GetParameter(2));
  mass->FixParameter(3, f->GetParameter(7));
  mass->FixParameter(4, f->GetParameter(8));
  ...
  std::cout << "4f get paramenter 0 " << f->GetParameter(0) << std::endl;
   
  TF1 *Bkpi = new TF1(Form("fBkpi%d",count),"[0]*("+iNP+")");
  Bkpi->SetParameter(0,f->GetParameter(9));
  ...
  Double_t yieldErr = mass->Integral(minhisto,maxhisto)/
    binwidthmass*mass->GetParError(0)/mass->GetParameter(0);
  ...
  leg->AddEntry(mass,"B^{_} Signal","f");
  leg->AddEntry(background,"Combinatorial","l");
  leg->AddEntry(Bkpi,"B #rightarrow J/#psi X","l");
  //leg->Draw("same");

  TLatex* texChi = new TLatex(0.58,0.55,
      Form("#chi^{2}/nDOF: %.2f/%d = %.2f", f->GetChisquare(), f->GetNDF(),
  ...
  TLatex* texCol;
  if(collisionsystem=="pp"||collisionsystem=="PP"||collisionsystem=="ppInc"
      ||collisionsystem=="PbPbInc") 
    texCol= new TLatex(0.96,0.93, Form("%s #sqrt{s_{NN}} = 5.02 TeV","pp"));
  else 
    texCol= new TLatex(0.96,0.93, Form("%s #sqrt{s_{NN}} = 5.02 TeV","PbPb"));
  texCol->SetNDC();
  ...
  if(centMax>0) {
    TString texper="%";
    tex = new TLatex(0.22,0.71,Form("Centrality %.0f-%.0f%s",
          centMin,centMax,texper.Data()));//0.2612903,0.8425793
    tex->SetNDC();
    tex->SetTextColor(1);
    tex->SetTextFont(42);
  ...
  if(isPbPb && isMC==0) 
      c->SaveAs(Form("plotFits/data_PbPb_%.0f_%.0f%s.pdf",
            ptmin,ptmax,_postfix.Data()));
  else if(isPbPb && isMC==1) 
      c->SaveAs(Form("plotFits/mc_PbPb_%.0f_%.0f%s.pdf",
            ptmin,ptmax,_postfix.Data()));
  else if(!isPbPb && isMC==0) 
      c->SaveAs(Form("plotFits/data_pp_%.0f_%.0f%s.pdf",
            ptmin,ptmax,_postfix.Data()));
  else 
      c->SaveAs(Form("plotFits/mc_pp_%.0f_%.0f%s.pdf",
            ptmin,ptmax,_postfix.Data()));
...
int main(int argc, char *argv[]) {
  if(argc==16) {
    fitBs(atoi(argv[1]),argv[2], argv[3], argv[4], argv[5], argv[6],
        atoi(argv[7]), atof(argv[8]), atoi(argv[9]),argv[10],argv[11],
        argv[12],atoi(argv[13]),atof(argv[14]),atof(argv[15]));
    return 0;
  }
  else if(argc==14) {
    fitBs(atoi(argv[1]), argv[2], argv[3], argv[4], argv[5], argv[6],
        atoi(argv[7]), atof(argv[8]), atoi(argv[9]),argv[10],argv[11],
        argv[12], atoi(argv[13]));
    return 0;
  }
  else {
...
double ErrorOnSigma(double width, double errwidth, double smear,
    double errsmearing){
double squarederroronsigma =
  (1+smear)*(1+smear)*errwidth*errwidth+width*width*errsmearing*errsmearing;
