...
int Bsdsigmadpt() {
  ...
  TGraphAsymmErrors* gae = 
    new TGraphAsymmErrors(REBIN, apt, asigma, aptl, aptl, aerrorl, aerrorh);
  ...
  for (int i=0;i<gaeSigmaDecay->GetN();i++){
    ...
    gaeSigmaDecay->SetPointEYhigh(
        i,gaeSigmaDecay->GetErrorYhigh(i)*BRFraction*norm);
    gaeSigmaDecay->SetPointEYlow(
        i,gaeSigmaDecay->GetErrorYlow(i)*BRFraction*norm); 
  }
  ...
  hempty->GetYaxis()->SetTitle("d#sigma/dp_{T}(B_{s}) #times A (pb c/GeV)");
  ...
  TLatex * tlatex=new TLatex(0.2,0.85,"B_{s},|y_{LAB}|<2.4, BR unc not shown");
  tlatex->SetNDC();
  tlatex->SetTextColor(1);
  tlatex->SetTextFont(42);
